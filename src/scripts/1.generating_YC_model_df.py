...
if __name__ == "__main__":
    ################################# Select X: independent variables (covariates for model)
    ...
    yield_corr = yields.groupby(['iso_a3', 'cropname', 'whichlag']).apply(
        lambda group: pd.Series({"rho": pearson_corr(group, x="csif_log_dt", y="yield_log_dt")})
    ).reset_index()
    yield_corr = yield_corr[yield_corr['rho'].abs() != 1]
    yield_corr['r2'] = yield_corr['rho'] ** 2

    ################################# Pull together final DF for modelling.
    ### Merge X covariates with R2 dependent variable
    dfs = [yield_corr, gdp, flag, ag, clim]

    merged = reduce(flexible_merge, dfs).dropna(subset=["r2","avg_sm"]).drop_duplicates().reset_index(drop=True)
    merged["flag_pct"] = merged["flag_pct"].fillna(0)

    coeffs = ["avg_gdp","flag_pct","total_harvarea","cropland_fraction","avg_sm","avg_tmax", "whichlag"]
    merged = merged.dropna(subset=coeffs + ["r2", "cropname"]).reset_index(drop=True)

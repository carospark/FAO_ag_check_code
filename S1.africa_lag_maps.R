...
lags <- read_csv("./data/africa_lags_update.csv") %>% drop_na()
...
ggplot() +
  ...
  geom_scatterpie(data=africa_lags,
    ...
    alpha = 0.7,
    pie_scale = 1  
  ) +  scale_fill_manual(values = newcolors, labels=c("No offset", "Year - 1 (lag)", "Year + 1 (lead)")) +
  theme_void() 
ggsave("./plots/africa_lags_piechart_update.pdf", width=5.7, height=6.3)
...
ggsave("./plots/africa_regions_piechart_update.pdf", width=5.7, height=6.3)
